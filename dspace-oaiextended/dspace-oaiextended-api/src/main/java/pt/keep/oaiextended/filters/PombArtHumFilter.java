...
public class PombArtHumFilter extends Filter {

        private static Logger log = Logger.getLogger(PombArtHumFilter.class);
	...
	public PombArtHumFilter () {
		
		// request set's
                //log.info(LogManager.getHeader(null, "PombArtHumFilter",null));
	...
	private List<String> getKnownPombalinaColl () {
		...
		try {
                        ...
                        String[] pombalina = ConfigurationManager.getProperty("oaiextended.filters.PombArtHumFilter.coll").split(",");

			if (pombalina != null) {
                            for (int i=0; i< pombalina.length; i++)

                                list.add(pombalina[i]);
                        log.info(LogManager.getHeader(null, "PombArtHumFilter:getKownPombalinaColl"," ## if pombalina not null"));
			}
                        // else log.info(LogManager.getHeader(null, "PombArtHumFilter:getKownPombalinaColl"," ## pombalina is null"));
		return list;
		} catch (Exception e) {
                     log.info(LogManager.getHeader(null, "PombArtHumFilter:getKownPombalinaColl",e.getLocalizedMessage()));
			return list;
		}
	}
	...
	public boolean isItemFiltered(AgentItem filterItem) {
		if (!filterItem.hasInformation()) return true;
                ...
                for (Collection collection : collections) {
                    log.info(LogManager.getHeader(null, "PombArtHumFilter:isItemFiltered # collection",collection.getHandle()));
		  ...
		  // DCValue[] fields = filterItem.getInformation().item.getMetadata(campo[0], campo[1], campo[2], Item.ANY);
               ...
               log.info(LogManager.getHeader(null, "PombArtHumFilter:isItemFiltered #",e.getLocalizedMessage()));
                }
		return (field || coll);
	}
	@Override
