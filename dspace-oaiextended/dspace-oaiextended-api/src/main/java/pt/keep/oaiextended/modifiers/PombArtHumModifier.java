...
public class PombArtHumModifier extends Modifier {
    	
	private static Logger log = Logger.getLogger(PombalinaModifier.class);
        private static String[] prefix;
        private static String[] index;

        public static final String DEFAULT_MODIFIERS = "pt.keep.oaiextended.modifiers.PombArtHumModifier";
	public static final String AGENT_GROUP_NAME = "modifiers";
	public static final String APPLICABLE_SETS_SPECS = "arteshumanidades";
	...
	public PombArtHumModifier () {
		...
		String[] array = super.getProperty("applicableSetsSpecs", APPLICABLE_SETS_SPECS).split(",");
           //     log.info(LogManager.getHeader(null,"PombArtHumFilter#array -",array[0]));
		...
		this.getFieldsPrefix();
           //     log.info(LogManager.getHeader(null, "PombArtHumModifier",null));
	}

 
	@Override
	public void modifyItem(AgentItem oaiItem) {
            // log.info(LogManager.getHeader(null, "PombArtHumModifier:modifyItem", null));
            DCValue[] values;
            if (this.index != null) {
                int size = this.index.length;
                log.info(LogManager.getHeader(null, "PombArtHumModifier:modifyItem:size #", String.valueOf(size)));
		if (oaiItem.hasInformation()) {
                try {
                 ...
                 for (int i=0; i< size; i++) {
                  ...
                  //log.info(LogManager.getHeader(null, "PombArtHumModifier:modifyItem:index #", String.valueOf(i) + " " + this.index[i]));
		  //log.info(LogManager.getHeader(null, "PombArtHumModifier:modifyItem:prefix #", String.valueOf(i) + " " + this.prefix[i]));
		  ...
		  for (DCValue value : values) {
		    ...
		    oaiItem.getInformation().item.addMetadata(value.schema, value.element, value.qualifier, value.language, this.prefix[i] + value.value);
                    //log.info(LogManager.getHeader(null, "PombArtHumModifier:modifyItem:metdata #",value.qualifier + "--" + value.value));
			}
		}
            }
            catch (Exception e) {
    ...
    public void getFieldsPrefix()
    {
        ...
        {
            log.info(LogManager.getHeader(null, "PombArtHumModifier:getFieldsPrefix:definition #", definition));
            definition2 = null;
            if ((definition2 = ConfigurationManager.getProperty("oaiextended.modifiers.UCdigitalisModifier.prefix." + idx))!=null) {
                log.info(LogManager.getHeader(null, "PombArtHumModifier:getFieldsPrefix:definition2 #", definition2));
        ...
        log.info(LogManager.getHeader(null, "PombArtHumModifier:getFieldsPrefix"," ## Fim"));
    }
}
